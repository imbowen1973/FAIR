// Does the picture go where the author is pointing?
//
  ...
  layouts: {
    Two: {
      ...
      regions: {
        ...
        left: { idx: 1, x: 0.06, y: 0.25, w: 0.42, h: 0.6, type: "body",
          style: { align: "l", sizePt: 20 }, inset: { l: 0.01, r: 0.01, t: 0.007, b: 0.007 } },
        right: { idx: 2, x: 0.52, y: 0.25, w: 0.42, h: 0.6, type: "object",
          style: { align: "l", sizePt: 20 }, inset: { l: 0.01, r: 0.01, t: 0.007, b: 0.007 } },
      },
    },
    Full: {
...
const FILES = {
  ...
  "blocks/01-b/block.yaml": "title: B\nduration_minutes: 45\nresources: []\n",
  "blocks/01-b/slides.md": [
    "---", "session: '01'", "title: B", "version: 1.0.0", "---", "",
    "--- slide", "id: s-01", "layout: Two", "title: First",
    "left: Some words on the left", "---", "",
    "--- slide", "id: s-02", "layout: Full", "title: Second",
    "full: Words here", "---", "",
...
// 3. The selection must not outlive its slide.
// A real click: the rail rows do not respond to a dispatched one.
await page.locator(".slide-row").nth(1).click();
await page.waitForTimeout(900);
const onSlideTwo = await regions();
const stillSelected = await page.evaluate(
  () => document.querySelector("#canvas .region.active")?.dataset.region ?? null
);
await pressPicture();
await page.waitForTimeout(350);
const afterMoving = await panelFor();
const saidAfterMoving = (await page.textContent("#status")).trim();
console.log(
  "slide 2:", JSON.stringify(onSlideTwo.map((r) => r.region)),
  "| still selected:", stillSelected,
  "| button aimed at:", afterMoving,
  "| said:", JSON.stringify(saidAfterMoving)
);
...
const failed =
  ...
  landed.left !== "Some words on the left" ||
  // Moving to another slide ends the selection rather than carrying a
  // region name into a layout that has no such placeholder.
  onSlideTwo.some((r) => r.region === "left" || r.region === "right") ||
  stillSelected !== null ||
  afterMoving !== null ||
  !/region/i.test(saidAfterMoving);
